            ...
            <CardHeader className="flex flex-row items-center justify-between gap-4 flex-wrap">
              <div>
                <CardTitle className="text-lg flex items-center gap-2">
                  ...
                  AI Executive Summary — {project.name}
                </CardTitle>
                <CardDescription>
                  A single consolidated summary covering all {versions.length} requirements for this project, ready to share with the client.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
              {versions.length === 0 ? (
                <div className="text-center p-12 text-muted-foreground">
                  ...
                  <p className="text-sm">No analysis available. Upload a BRD document first.</p>
                </div>
              ) : (() => {
                  const summary = versions.find(v => v.ai_summary)?.ai_summary;
                  if (!summary) {
                    return (
                      <div className="p-8 border rounded-lg text-center bg-muted/10 text-muted-foreground">
                        ...
                        <p className="text-sm">AI summary is processing or was not generated. Try re-uploading the BRD document.</p>
                      </div>
                    );
                  }
                  return (
                    <div className="p-6 border rounded-xl bg-white shadow-sm space-y-5">
                      {/* Header strip */}
                      <div className="flex items-center justify-between border-b pb-3 flex-wrap gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className="text-muted-foreground">
                            {versions.length} Requirements Covered
                          </Badge>
                        </div>
                      </div>

                      {/* The single summary text */}
                      <div className="prose prose-sm max-w-none text-foreground whitespace-pre-wrap leading-relaxed bg-muted/5 border rounded-lg p-5">
                        {summary}
                      </div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground pt-2 border-t">
                        ...
                        This summary was generated by AI from the full BRD document and covers all extracted requirements for <strong>{project.name}</strong>.
                      </div>
                    </div>
                  );
              })()}
